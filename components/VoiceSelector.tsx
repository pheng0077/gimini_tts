...
import { getVoiceSample, cacheVoiceSample } from '../services/voiceCache';
...
export const VoiceSelector: React.FC<VoiceSelectorProps> = ({ voices, selectedVoiceId, onSelect, isLoading, userApiKey }) => {
    ...
    const handlePreview = async (e: React.MouseEvent, voice: VoiceOption) => {
        ...
        try {
            ...
            console.log(`Checking cache for ${voice.name}...`);
            const cachedUrl = await getVoiceSample(voice.name);

            let audioUrl: string;

            if (cachedUrl) {
                // Cache hit! Use the cached sample
                console.log(`✓ Cache hit for ${voice.name}`);
                audioUrl = cachedUrl;
            } else {
                // Cache miss - generate new sample
                console.log(`✗ Cache miss for ${voice.name} - generating...`);
                ...
                // Upload to cache for next time (don't await - let it happen in background)
                cacheVoiceSample(voice.name, wavBlob).then((success) => {
                    if (success) {
                        console.log(`✓ Cached sample for ${voice.name}`);
                    } else {
                        console.log(`✗ Failed to cache sample for ${voice.name}`);
                    }
                });
            }
            ...
            if (errorMessage.includes('429') || errorMessage.toLowerCase().includes('rate limit') || errorMessage.toLowerCase().includes('quota')) {
                alert('⚠️ Rate Limit Reached\n\nGemini TTS has very strict rate limits (1-2 requests/min).\n\nTip: Voice previews are optional - you can select a voice without previewing it. The main TTS generation will work fine.');
            } else {
                        ...
                        {filteredVoices.map(voice => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`flex items-center p-3 cursor-pointer transition-colors border-l-2 ${isSelected
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        className={`w-8 h-8 rounded-full border flex items-center justify-center shrink-0 mr-3 transition-all ${isPlaying
